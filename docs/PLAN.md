...
*Design reference: **Buffer** — clone the UI/UX **100%** for v1 (clean single-column composer +
queue/calendar), then tweak/brand it afterward.*
...
| CMS (optional) | **Sanity** — *Phase 5 only* | Not needed for MVP (see §10) |
...
once LinkedIn/Instagram publish, the image/video is hosted on *their* servers, so the live post
...
- **Permanent history (decided):** the purge is a *working-storage* policy, not total amnesia.
  ...
  of *what was posted and a link to it*, while the heavy stuff (text body, images, videos) is still
  purged at 7 days. No media is retained — just the lightweight pointer to the live post.

**Caveats to remember:**
- GitHub cron is *best-effort* — can be delayed/skipped a few minutes under load. Fine for social posts.
...
**Instagram (Meta Graph API)** — *verified against Meta docs, June 2026*

Meta offers **two connection methods**. We use the newer, simpler one as primary.

- **PRIMARY → Instagram API with *Instagram Login*** (launched July 2024):
  - **No Facebook Page required.** Client just needs an Instagram **Professional** account
    (Business *or* Creator — either works). They log in with Instagram credentials directly.
...
- **FALLBACK → Instagram API with *Facebook Login*** (older):
...
- Personal (non-Professional) IG accounts can't auto-post via *any* method → fallback "reminder to
