...
| #   | File                         | Issue                                                                                                                                                                                                                     | Fix                                                                              |
| --- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| 1   | `api/cron/cleanup/route.ts`  | **Data loss.** Daily purge deleted _any_ post older than 7 days by `createdAt` - including drafts, posts awaiting approval, and **future-scheduled** posts (old `createdAt`, future `scheduledAt`) - plus their R2 media. | Only purge terminal posts (`PUBLISHED`/`PARTIAL`/`FAILED`).                      |
...
- **Full `htmlFor`/`id` association on _every_ Label/Input pair** (finding 5). We covered the
